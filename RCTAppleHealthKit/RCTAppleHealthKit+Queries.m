...
- (void)fetchDiscreteAverageStatisticsCollection:(HKQuantityType *)quantityType
                                            unit:(HKUnit *)unit
                                       startDate:(NSDate *)startDate
                                         endDate:(NSDate *)endDate
                                        interval:(NSDateComponents *)interval
                                      completion:(void (^)(NSArray *, NSError *))completionHandler {
	[self fetchStatisticsCollection:quantityType
							options:HKStatisticsOptionDiscreteAverage
							   unit:unit
						  startDate:startDate
							endDate:endDate
						   interval:interval
							handler:^(HKStatistics* result) { return result.averageQuantity; }
						 completion:completionHandler
	];
}

- (void)fetchMinStatisticsCollection:(HKQuantityType *)quantityType
								unit:(HKUnit *)unit
						   startDate:(NSDate *)startDate
							 endDate:(NSDate *)endDate
							interval:(NSDateComponents *)interval
						  completion:(void (^)(NSArray *, NSError *))completionHandler
{
	[self fetchStatisticsCollection:quantityType
						  ...
						  startDate:startDate
							endDate:endDate
						   interval:interval
							handler:^(HKStatistics* result) { return result.minimumQuantity; }
						 completion:completionHandler
	];
}

- (void)fetchMaxStatisticsCollection:(HKQuantityType *)quantityType
								unit:(HKUnit *)unit
						   startDate:(NSDate *)startDate
							 endDate:(NSDate *)endDate
							interval:(NSDateComponents *)interval
						  completion:(void (^)(NSArray *, NSError *))completionHandler
{
	[self fetchStatisticsCollection:quantityType
						  ...
						  startDate:startDate
							endDate:endDate
						   interval:interval
							handler:^(HKStatistics* result) { return result.maximumQuantity; }
						 completion:completionHandler
	];
}

- (void)fetchStatisticsCollection:(HKQuantityType *)quantityType
						  options:(HKStatisticsOptions)options
							 unit:(HKUnit *)unit
						startDate:(NSDate *)startDate
						  endDate:(NSDate *)endDate
						 interval:(NSDateComponents *)interval
						  handler:(HKQuantity* (^)(HKStatistics*))handlerBlock
					   completion:(void (^)(NSArray *, NSError *))completionHandler {
	NSCalendar *calendar = [NSCalendar currentCalendar];
	NSDateComponents *anchorComponents = [calendar components:NSCalendarUnitDay | NSCalendarUnitMonth | NSCalendarUnitYear
													 fromDate:[NSDate date]];
	...
	query.initialResultsHandler = ^(HKStatisticsCollectionQuery *query, HKStatisticsCollection *results, NSError *error) {
		...
		[results enumerateStatisticsFromDate:startDate
									  toDate:endDate
								   withBlock:^(HKStatistics *result, BOOL *stop) {
			HKQuantity *quantity = handlerBlock(result);
			if (quantity) {
				double value = [quantity doubleValueForUnit:[HKUnit countUnit]];
